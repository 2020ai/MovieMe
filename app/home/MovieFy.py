...
class MovieFy:
    ...
    def get_ratings_for_movie_ids(self, user_id, movie_ids):
        ...
        """
        requested_movies_RDD = self.sc.parallelize(
            movie_ids).map(lambda x: (user_id, x))
    ...
    def get_top_ratings(self, user_id, movies_count):
        ...
        # Get predicted ratings
        ratings = self.predict_ratings(user_unrated_movies_RDD).filter(
    ...
    def run(self, new_user_ratings):
      ...
      """
      try:
        self.add_ratings(new_user_ratings)
        return self.get_top_ratings(0, 25)
...
#         movieFy = MovieFy(dataset_path)
#         movieFy.run(new_user_ratings)
